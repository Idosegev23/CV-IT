  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error: signUpError } = await supabase.auth.signUp({
        ...
        options: {
          data: {
            role: 'admin' // מגדיר את המשתמש כאדמין
          }
        }
      });
      ...
      router.push(`/${lang}/admin/login?registered=true`);
      ...
      <div className="max-w-md w-full space-y-8">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600">
            או{' '}
            <Link href={`/${lang}/admin/login`} className="font-medium text-blue-600 hover:text-blue-500">
              התחברות למערכת
            </Link>
          </p>
        </div>
